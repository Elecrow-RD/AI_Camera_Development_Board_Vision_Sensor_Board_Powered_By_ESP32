    ...
    LGFX(void) {
      {
        ...
        cfg.freq_write = 60000000;          // 传输时的SPI时钟（最高80MHz，四舍五入为80MHz除以整数得到的值）
        ...
        cfg.pin_mosi = 13;  // SPIのCLKピン番号を設定
        cfg.pin_miso = -1;  // SPIのMISOピン番号を設定 (-1 = disable)
        cfg.pin_dc = 9;     // 设置 SPI D/C 引脚编号。  (-1 = disable)
      ...
      { // 表示パネル制御の設定を行います。
        ...
        cfg.memory_height = 284;   // ドライバICがサポートしている最大の高さ
        cfg.panel_width = 240;     // 実際に表示可能な幅
        cfg.panel_height = 284;    // 実際に表示可能な高さ
        cfg.offset_x = 0;          // パネルのX方向オフセット量
        cfg.offset_y = 0;         // パネルのY方向オフセット量
        cfg.offset_rotation = 0;   //值在旋转方向的偏移0~7（4~7是倒置的）
        cfg.dummy_read_pixel = 16;  // 在读取像素之前读取的虚拟位数
      ...
      { // タッチスクリーン制御の設定を行います。（必要なければ削除）
        ...
        cfg.y_max = 283;          // タッチスクリーンから得られる最大のY値(生の値)
        cfg.pin_int = 11;         // INTが接続されているピン番号
        cfg.bus_shared = false;   // 如果您使用与屏幕相同的总线，则设置为 true
        cfg.offset_rotation = 2;  // 显示和触摸方向不匹配时的调整 设置为 0 到 7 的值
