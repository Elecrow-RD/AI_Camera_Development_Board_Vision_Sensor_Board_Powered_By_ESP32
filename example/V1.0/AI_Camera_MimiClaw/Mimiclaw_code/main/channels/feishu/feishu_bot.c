...
#if 1
        int64_t last_ping = 0;
        int64_t last_alive_time = esp_timer_get_time() / 1000;
        while (s_ws_client)
        {
            int64_t now = esp_timer_get_time() / 1000;
            bool is_physically_connected = esp_websocket_client_is_connected(s_ws_client);
            if (s_ws_connected || is_physically_connected)
            {
                last_alive_time = now;
                if (s_ws_connected)
                {
            ...
            {
                if (now - last_alive_time > 30000)
                {
                    ESP_LOGE(TAG, "Connect timeout, breaking loop");
                    break;
                }
            }
